...
# loading airpots data
AIRPORTS = airportsdata.load("IATA")


# when user passes alias info
COUNTRY_ALIASES = {
    "usa": "US",
    "u.s.a": "US",
    "u.s.": "US",
...
def clean_text(text: str) -> str:
    ...
    text = re.sub(r"[^a-z0-9\s]", " ", text) #removing special char, replacing it with space
    ...
    # spliting sentences into words and removing stop words
    words = [w for w in text.split() if w not in stop_words]
    return " ".join(words).strip() # join back the words


# getting code from country name
...
# input: airport + country code
# output: check if airport belongs to the cpuntry

# get airport country and match with country code
...
# Take whatever location the user gives and convert it into an airport's 3-letter IATA code.
...
# takes the user's natural-language flight query and figures out the departure and arrival airport IATA codes.
