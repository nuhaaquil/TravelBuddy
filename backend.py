...
# function for llm call to avoid writing code again
...
"""
    ...
    try:
        # llm call using the created function
        guardrail_raw = _llm_text(
            "You are the input guardrail for a travel-planning application. "
            "Return strict JSON only.",
            guardrail_prompt,
        )
        guardrail_result = _json_from_llm(guardrail_raw) # convert result to json
...
"""
    ...
    try:
        ...
        # if itinerary agent is not in the selected agents, add it to the list manually because it is requried
        if "itinerary_agent" not in selected_agents:
            selected_agents.append("itinerary_agent")

        # extract constraints from user query and convert to dict
        constraints = _empty_constraints()
        parsed_constraints = parsed.get("trip_constraints", {})
        if isinstance(parsed_constraints, dict):
            constraints.update(parsed_constraints)

        # reason for selecting/rejecting the agents for the query
...
# Flight Tool Router Prompt Template
FLIGHT_AGENT_PROMPT = """
...
def flight_agent(state: TravelState):
    ...
    try:
        ...
        airports = asyncio.run(
            aviation_mcp_call(
                "list_airports" # invoking this tool
            )
        )

        airlines = asyncio.run(
            aviation_mcp_call(
                "list_airlines" #invoking this tool
            )
        )
